...
export default function OgCollection() {
  ...
  const toriiGateRef = useRef<HTMLDivElement>(null);
  ...
  // Effect for GSAP animation of Torii gate
  useEffect(() => {
    if (toriiGateRef.current) {
      gsap.fromTo(
        toriiGateRef.current,
        { y: 100, opacity: 0 }, // Start position: 100px below and invisible
        {
          ...
          scrollTrigger: {
            trigger: sectionRef.current, // Trigger animation when OgCollection section is in view
            start: 'top 80%', // Start animation when top of section is 80% from the top of viewport
            end: 'top 50%', // End animation when top of section is 50% from the top of viewport
  ...
  return (
    <section
      ...
      className="relative w-full h-[1450px] bg-black overflow-hidden"
    >
      {/* Black background */}
      <div className="absolute inset-0 bg-black z-0"></div>

      {/* Torii Gate with ref for GSAP animation */}
      <div ref={toriiGateRef} className='absolute inset-0 flex justify-center items-start z-4'>
        <Image src="/toriigate.webp" alt="toriigate" width={300} height={300} />
      </div>

      <div className='absolute inset-0 flex justify-center items-start z-4 lg:scale-11 0'>
      ...
      <div
        className="relative z-10 w-full h-full flex justify-center items-center group mt-15 -lg:mt-10"
      ...
      >
        ...
        <video
          ...
          poster="/V2.webp"
          className="w-[90%] h-auto max-w-full shadow-lg cursor-pointer lg:mt-100 " // Áp dụng w-[90%], giữ tỷ lệ, bo góc, thêm bóng đổ
        ...
        <Image src="/video_frame.png" alt='videoframe' width={1920} height={1080} className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] h-auto max-w-full pointer-events-none z-15 lg:mt-50'/>

         {/* Nút Play/Pause tùy chỉnh */}
        <div className={`
            absolute inset-0 flex items-center justify-center z-20
            rounded-lg pointer-events-none translate-y-50
